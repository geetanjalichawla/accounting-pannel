...
export default function FlashReport() {
  ...
  const fetchData = async (download) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_FRONTEND}/daily-report?client=tacobell&date=${date}&download=${download}`
      );
  ...
  return (
    <div>
      ...
      <Text fontSize='2xl'>Daily Flash Report</Text>
      {/* </Box>
      </Center> */}

      <>
        <RestaurantGroupReport summary={receivedSummary}/>
        <AreaSummeryReport aggregatedData={receivedArray} summaryData={receiveSummary}/>
        <ManagerSales
          manager='Fabiola Theodore'
          sendArrayToParent={receiveArray}
          data={data}
        />
        <ManagerSales
