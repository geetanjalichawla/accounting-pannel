...
const SalesReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND}/weekly-report/weekly-mclanepurchase?week=${week}&period=${period}&year=${year}`);
  ...
  return (
    <Box pt={{ base: "130px", md: "80px", xl: "80px" }} w="100%" bg={'white'}>
      ...
      <Heading size='xl' mb={'2'}>Weekly  mclane purchase Report</Heading>
      {data && (
