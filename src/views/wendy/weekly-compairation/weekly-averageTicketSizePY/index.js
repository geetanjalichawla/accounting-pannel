...
const SalesReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND}/wendys/weekly-report/weekly-averageTicketSizePY?week=${week}&period=${period}&year=${year}`);
  ...
  const handleDownload = () => {
    if (data) {
        ...
        Object.keys(reporti).forEach((reportKey) => {
          ...
          sheetData = [
            ...
            [
              ...
              'Total Sales',
              ...(report?.currentReport?.report.map((entry) => parseFloat(entry.averageTicketSizePY?.toFixed(2)) || 0) ?? []),
            ],
            [
              ...
              'Total Sales',
              ...(report?.prevReport?.report.map((entry) => parseFloat(entry.averageTicketSizePY?.toFixed(2)) || 0) ?? []),
            ],
            [
              '--',
              '--',
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'averageTicketSizePY');

      XLSX.writeFile(wb, 'averageTicketSizePY.xlsx');
    }
  };
  return (
    <Box pt={{ base: "130px", md: "80px", xl: "80px" }} w="100%" bg={'white'} p="2">
      ...
      <Heading size='xl' mb={'2'}>average Ticket Size PY</Heading>
      {data && (
                    ...
                    <Table>
                      ...
                      <Tbody>
                        <Tr>
                          ...
                          {data.data[type][type2].currentReport.report.map((entry) => (
                            <Td minW={"max-content"} key={entry.storeName}>{parseFloat(entry.averageTicketSizePY?.toFixed(2)) || 0}</Td>
                          ))}
                        </Tr>
                        <Tr>
                          {
