...
const SalesReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND}/wendys/weekly-report/weekly-doordash?week=${week}&period=${period}&year=${year}`);
        ...
        Object.keys(reporti).forEach((reportKey) => {
          ...
          sheetData = [
            ...
            [
              ...
              'Total Sales',
              ...(report?.currentReport?.report.map((entry) => parseFloat(entry.doordash?.toFixed(2)) || 0) ?? []),
            ],
            [
              ...
              'Total Sales',
              ...(report?.prevReport?.report.map((entry) => parseFloat(entry.doordash?.toFixed(2)) || 0) ?? []),
            ],
            [
              '--',
              '--',
  ...
  return (
    <Box pt={{ base: "130px", md: "80px", xl: "80px" }} w="100%" bg={'white'} p="2">
      ...
      <Heading size='xl' mb={'2'}>doordash</Heading>
      {data && (
                    ...
                    <Table>
                      ...
                      <Tbody>
                        <Tr>
                          ...
                          {data.data[type][type2].currentReport.report.map((entry) => (
                            <Td minW={"max-content"} key={entry.storeName}>{parseFloat(entry.doordash?.toFixed(2)) || 0}</Td>
                          ))}
                        </Tr>
                        <Tr>
                          {
