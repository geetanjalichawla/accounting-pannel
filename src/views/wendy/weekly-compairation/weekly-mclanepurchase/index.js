...
const SalesReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND}/wendys/weekly-report/weekly-foodpurchase?week=${week}&period=${period}&year=${year}`);
        ...
        Object.keys(reporti).forEach((reportKey) => {
          ...
          sheetData = [
            ...
            [
              ...
              'Total Sales',
              ...(report?.currentReport?.report.map((entry) => parseFloat(entry.foodPurchase?.toFixed(2)) || 0) ?? []),
            ],
            [
              ...
              'Total Sales',
              ...(report?.prevReport?.report.map((entry) => parseFloat(entry.foodPurchase?.toFixed(2)) || 0) ?? []),
            ],
            [
              '--',
              '--',
                    ...
                    <Table>
                      ...
                      <Tbody>
                        <Tr>
                          ...
                          {data.data[type][type2].currentReport.report.map((entry) => (
                            <Td minW={"max-content"} key={entry.storeName}>{parseFloat(entry.foodPurchase?.toFixed(2)) || 0}</Td>
                          ))}
                        </Tr>
                        <Tr>
                          {
