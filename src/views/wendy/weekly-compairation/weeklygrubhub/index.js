...
const SalesReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND}/wendys/weekly-report/weeklygrubhub?week=${week}&period=${period}&year=${year}`);
        ...
        Object.keys(reporti).forEach((reportKey) => {
          ...
          sheetData = [
            ...
            [
              ...
              'Total Sales',
              ...(report?.currentReport?.report.map((entry) =>  parseFloat(entry.grubhub.toFixed(2))) ?? []),
            ],
            [
              ...
              'Total Sales',
              ...(report?.prevReport?.report.map((entry) => parseFloat(entry.grubhub.toFixed(2))) ?? []),
            ],
            [
              '--',
              '--',
              ...
              '--',
              'Variance($)',
              ...(report.variancedata.map((entry) => parseFloat(entry.totalSalesdiff.toFixed(2))) ?? []),
            ],
            [
              '--',
              '--',
              ...
              '--',
              'Variance(%)',
              ...(report.variancedata.map((entry) =>  parseFloat(entry.variancePercentage.toFixed(2))) ?? []),
            ],
          ];
        });
      });
        ...
        <Table>
                ...
                <Tbody>
                  <Tr>
                    ...
                    {data.data[type][type2].currentReport.report.map((entry) => (
                      <Td minW={"max-content"} key={entry.storeName}>{parseFloat(entry.grubhub.toFixed(2))}</Td>
                    ))}
                  </Tr>
                  <Tr>
                    {
                  ...
                  <Tr>
                    ...
                    {data.data[type][type2].variancedata.map((entry) => (
                      <Td minW={"max-content"} key={entry.storeName}>{ parseFloat(entry.totalSalesdiff.toFixed(2))}</Td>
                    ))}
                  </Tr>
                  <Tr>
                    {
                    ...
                    <Th>Variance(%)</Th>
                    {data.data[type][type2].variancedata.map((entry) => (
                      <Td key={entry.storeName}>{ parseFloat(entry.variancePercentage.toFixed(2))}</Td>
                    ))}
                  </Tr>
                </Tbody>
              </Table>
